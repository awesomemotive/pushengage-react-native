...
interactions with the project. Before contributing, please read the
...
Objective-C or Swift files, open `example/ios/PushengageReactNativeExample.xcworkspace` in XCode and
find the source files at `Pods > Development Pods > pushengage-react-native`.

To edit the Java or Kotlin files, open `example/android` in Android studio and find the source files
at `pushengage-react-native` under `Android`.
...
```sh
Running "PushengageReactNativeExample" with {"fabric":true,"initialProps":{"concurrentRoot":true},"rootTag":1}
```
...
[ESLint](https://eslint.org/) with [Prettier](https://prettier.io/) for linting and formatting the
code, and [Jest](https://jestjs.io/) for testing.
